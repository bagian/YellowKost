...
<div class="max-w-2xl pt-20 mx-auto">
    <form action="@if($testimonial->isEmpty()) {{ route('testimonial.store') }} @else {{ route('testimonial.update', $testimonial->id) }} @endif" method="POST">
        @csrf
        <div class="space-y-6">
            <div class="bg-white border border-gray-200 shadow-lg rounded-2xl dark:border-gray-800 dark:bg-gray-800">
                ...
                <div class="p-5 space-y-6 border-t border-gray-100 sm:p-6 dark:border-gray-700">
                    ...
                    <div>
                        ...
                        <input type="text" id="full_name" name="full_name" placeholder="Masukkan nama Anda" value="{{ auth()->user()->full_name }}"
                    ...
                    <div>
                        ...
                        <input type="text" id="occupation" name="occupation" placeholder="Contoh: Karyawan Swasta" value="{{ auth()->user()->occupation }}"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                            required>
                    </div>
                    <!-- Rating Bintang -->
                    <div x-data="{ rating: {{ $testimonial->rating ?? 0 }}, hoverRating: {{ $testimonial->rating ?? 0 }} }">
