...
<div class="flex flex-col items-center max-w-5xl pt-20 mx-auto">
    <div class="p-6 text-xs rounded-lg bg-slate-300/30 line-clamp-2">
        <div class="flex flex-row gap-2">
            ...
            <span class="block pt-0 m-0">
                Silakan unggah bukti pembayaran Anda untuk memproses transaksi lebih lanjut. Pastikan file yang diunggah
                dalam format PNG, JPG, atau JPEG dengan ukuran maksimal 5MB. Harap unggah bukti pembayaran yang jelas
                dan
                dapat dibaca agar proses verifikasi dapat berjalan lancar.
            </span>
        </div>
    </div>
    <div class="w-full max-w-lg px-4 pt-6">
        <div
                ...
                <form action="{{ route('booking.payment.submit') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div>
                        <label id="drop-zone" for="paymentProof"
                            ...
                            {{-- Preview State --}}
                            <div id="preview-state" class="hidden w-full text-center">
                                <img id="image-preview" class="object-contain h-48 mx-auto mb-4 rounded-lg"
                                    alt="Pratinjau Gambar" />
                                <p id="file-name"
                                    class="overflow-hidden text-sm font-medium text-gray-800 dark:text-gray-200 text-ellipsis whitespace-nowrap">
                                </p>
...
@push('scripts')
{{-- Pastikan jQuery sudah dimuat sebelum skrip ini, misalnya di file layout utama Anda --}}
<script>
